...
from routers import openrouter_client
...
def _build_system_prompt(current_user: dict, pos_data: Dict[str, Any]) -> str:
    """Build a system prompt that gives the LLM the business context and live POS data."""
    business_name = current_user.get("business_name") or current_user.get("username")
    role = current_user.get("role", "admin")

    data_json = json.dumps(pos_data, default=str, indent=2)

    return (
        "You are the AI assistant embedded in Ailexity POS, a restaurant/retail point-of-sale system. "
        f"You are talking to {business_name} (role: {role}). "
        "Answer questions about their business using ONLY the JSON data provided below, which was just "
        "fetched live from their database. Be concise, friendly, and use currency symbol Rs. for amounts. "
        "You can analyze trends, compare periods, highlight top items, flag low stock or overdue parties, "
        "and give actionable suggestions. If the data needed to answer isn't present in the JSON, say so "
        "honestly instead of making up numbers. Format numeric answers clearly, and use short paragraphs "
        "or bullet points where helpful.\n\n"
        f"LIVE POS DATA (JSON):\n{data_json}"
    )
...
async def query_ai_assistant(
    ...
    try:
        ...
        # Prefer a real LLM (OpenRouter) for personalized, full-data-aware answers
        if openrouter_client.is_configured():
            try:
                system_prompt = _build_system_prompt(current_user, pos_data)
                history = [{"role": m.role, "content": m.content} for m in query.history[-10:]]
                response_text = await openrouter_client.chat_completion(
                    system_prompt, history, query.message
                )
