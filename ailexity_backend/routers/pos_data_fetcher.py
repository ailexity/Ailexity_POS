...
class POSDataFetcher:
    ...
    def fetch_pos_data(self, date: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            data = {
                ...
                # Parties (customers/suppliers) ledger summary
                'parties_summary': self._fetch_parties_summary(),

                # Raw stock / inventory summary
                'raw_stock_summary': self._fetch_raw_stock_summary(),
            }
    ...
    def _fetch_parties_summary(self) -> Dict[str, Any]:
        """Fetch a summary of customers/suppliers and outstanding balances."""
        try:
            import database

            party_filter = {} if self.admin_id is None else {"admin_id": self.admin_id}
            parties = list(database.parties_collection.find(party_filter))

            total_receivable = sum(p.get("total_receivable", 0) for p in parties)
            total_payable = sum(p.get("total_payable", 0) for p in parties)

            top_debtors = sorted(
                (p for p in parties if p.get("current_balance", 0) > 0),
                key=lambda p: p.get("current_balance", 0),
                reverse=True
            )[:5]
            ...
            return {
                ...
                'top_debtors': [
                    {'name': p.get("party_name"), 'balance': p.get("current_balance", 0)}
                    for p in top_debtors
                ],
            }
        except Exception as e:
            logger.error(f"Error fetching parties summary: {e}", exc_info=True)
            return {}

    def _fetch_raw_stock_summary(self) -> Dict[str, Any]:
        """Fetch a summary of raw stock / inventory items, highlighting low stock."""
        try:
            import database

            stock_filter = {} if self.admin_id is None else {"admin_id": self.admin_id}
            raw_stock = list(database.raw_stock_collection.find(stock_filter))

            low_stock = [
                {'name': s.get("name"), 'currentStock': s.get("currentStock", 0), 'reorderLevel': s.get("reorderLevel", 0), 'unit': s.get("unit")}
                for s in raw_stock
                if s.get("currentStock", 0) <= s.get("reorderLevel", 0)
            ]

            return {
                'total_raw_stock_items': len(raw_stock),
                'low_stock_items': low_stock,
                'low_stock_count': len(low_stock),
            }
        except Exception as e:
            logger.error(f"Error fetching raw stock summary: {e}", exc_info=True)
            return {}
